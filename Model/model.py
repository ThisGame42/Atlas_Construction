...
class VxmDenseV2(LoadableModel):
    """
    ...
    def __init__(self,
        ...
        self.flow_img_fw = Conv(self.unet_forward.dec_nf[-1], ndims, kernel_size=3, padding=1)
        ...
        self.flow_label_fw_weight = nn.Parameter(torch.ones(size=(1, 1, 128, 240, 240)),
                                                 requires_grad=True)

        self.flow_img_bk = Conv(self.unet_forward.dec_nf[-1], ndims, kernel_size=3, padding=1)
        self.flow_img_bk.weight = nn.Parameter(Normal(0, 1e-5).sample(self.flow_img_fw.weight.shape))
        self.flow_img_bk.bias = nn.Parameter(torch.zeros(self.flow_img_fw.bias.shape))
        self.flow_label_bk_weight = nn.Parameter(torch.ones(size=(1, 1, 128, 240, 240)),
                                                 requires_grad=True)
    ...
    def forward(self, moving_img, fixed_img, moving_label, fixed_label, registration=True):
        """
        Parameters:
        ...
        features_fw = self.unet_forward(x_fw)

        # transform into flow field
        flow_field_fw = self.flow_img_fw(features_fw)
        moved_img, img_flow_fw = self.register_tensor(flow_field_fw,
                                                      moving_tensor=moving_img,
                                                      fixed_tensor=fixed_img,
                                                      registration=registration)
        flow_field_label_fw = flow_field_fw
        moved_label, label_flow_fw = self.register_tensor(flow_field_label_fw,
                                                          moving_tensor=moving_label,
                                                          fixed_tensor=fixed_label,
                                                          registration=registration)

        x_bk = torch.cat([fixed_img, moving_img], dim=1)
        features_bk = self.unet_backward(x_bk)

        flow_field_bk = self.flow_img_bk(features_bk)
        moved_img_warped, img_flow_bw = self.register_tensor(flow_field_bk,
                                                             moving_tensor=fixed_img,
                                                             fixed_tensor=moving_img,
                                                             registration=registration)
        flow_field_label_bk = flow_field_bk
        moved_label_warped, label_flow_bk = self.register_tensor(flow_field_label_bk,
                                                                 moving_tensor=fixed_label,
                                                                 fixed_tensor=moving_label,
                                                                 registration=registration)
        return {
            "moved_img": moved_img,
            "fixed_img_moved": moved_img_warped,
            "moved_label": moved_label,
            "fixed_label_moved": moved_label_warped,
            "img_flow_fw": img_flow_fw,
            "img_flow_bk": img_flow_bw,
            "img_flow_reversible": img_flow_fw + img_flow_bw
        }
...
if __name__ == "__main__":
    model = VxmDenseV2(3, ndims_c=4, inshape=(32, 240, 240),
                       nb_unet_features=[
                           [32, 32, 32, 32],
                           [32, 32, 32, 32, 32, 16],
                       ], bidir=True).cuda()
    x1 = torch.randn(4, 4, 32, 240, 240).cuda()
    x2 = torch.randn(4, 4, 32, 240, 240).cuda()
    x3 = torch.randn(4, 12, 32, 240, 240).cuda()
    x4 = torch.randn(4, 12, 32, 240, 240).cuda()
    results = model(x1, x2, x3, x4)
    print(results["moved_label"][:, 1:, ...].size())
